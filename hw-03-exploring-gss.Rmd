...
***There are three possible responses: Yes, No, and NA. In total, 96 respondents chose it does not apply to them, 1136 respondents chose No, 237 respondents chose Yes, and 1398 respondents chose NA.***
...
gss16 %>%
  ...
  nrow()
```
***43% of the respondents have been harassed by superior or co-workers at their job.***
...
***The mean number of minutes spent on email per week is approx 416.8 minutes and the median number of minutes spent on email per week is 120 minutes. This data is skewed so the mean is the better measure when finding the typical time Americans spend on email weekly.***
...
***37 percent of people answered "Yes" for snap_insta.***
...
***284 respondents are keeping house, 76 respondents are going to school, 345 respondents are working part time, and 1321 respondents are working full time.***
...
gss16 <- gss16 %>%
  mutate(
    polyviews2 = case_when(
      polviews %in% c("Extremely liberal", "Liberal", "Slightly liberal") ~ "Liberal",
      polviews %in% c("Extremely conservative", "Conservative", "Slightly conservative") ~ "Conservative", TRUE             ~ polviews
      ),
    polviews2 = fct_relevel(polyviews2, "Conservative", "Moderate", "Liberal")
  )
```
...
```{r}
gss16 %>%
  filter(!is.na(polyviews2), !is.na(advfront2)) %>%
  ggplot(aes(x = polyviews2, fill = advfront2)) +
